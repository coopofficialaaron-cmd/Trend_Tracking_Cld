...
function renderMarket(){
  ...
  const keys=Object.keys(DATA.market||{}).filter(b=>DATA.market[b]).sort();
  keys.forEach(b=>{
...
function fillDatalists(){
  ...
  const set=(id,vals)=>document.getElementById(id).innerHTML=vals.map(v=>`<option value="${v}">`).join("");
  set("benchList",uniq("benchmark")); set("majorList",uniq("major")); set("subList",uniq("sub"));
}
function openAdd(){
  fillDatalists();
