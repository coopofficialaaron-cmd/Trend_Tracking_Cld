...
/* 排序键 = ATR%,不再是"档位 ×1000 + 止损%"。
   旧设计的依据是全样本上"止损% 越宽 PF 越高"(<5% → 1.19,18%+ → 2.81)。那个规律是真的,
   但它是共线的影子:止损% ≈ 4 × ATR%,18% 以上那一档 100% 都是强档。
   一旦先用「强/中 + 止损% ≥ 8%」筛过,共线信息已被抽干,剩下的止损% 变异是噪音——
...
function renderMarket(){
  ...
  if(slot){
    ...
    if(bd){
      ...
      div.title=`${bd.ok}/${bd.n} 个标的的基准 ETF 向上`;
      slot.appendChild(div);
    }
  }
  // 不过滤无数据的对标：抓取失败时它会整条消失，页面上看不出异常，
  // 而挂靠其下的股票会全部静默判为 Bad Market。改为渲染成警示 pill 并排在最前。
...
function chartHTML(st){
  ...
  data.forEach((p,i)=>{ const ym=p.date.slice(0,7);
    if(ym!==lastM){ lastM=ym;
      const lab = MON[(+p.date.slice(5,7))-1]+p.date.slice(2,4);   // Aug26（与持仓页一致）
