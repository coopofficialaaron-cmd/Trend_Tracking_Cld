...
const ER_HOT = 0.30;      // ER22 或 ER55 < 0.3
...
function hotReasons(s){
  ...
  if(s.er22!=null && s.er22<ER_HOT) r.push(`ER22 ${fmt.er(s.er22)} < ${ER_HOT}`);
  if(s.er55!=null && s.er55<ER_HOT) r.push(`ER55 ${fmt.er(s.er55)} < ${ER_HOT}`);
  return r;
}
...
const COLS = [
  {k:"ticker",  t:"代码", l:true,  f:(s,st)=>`${freshDot(st)}<span class="tk">${st.ticker}</span>`, v:(s,st)=>st.ticker},
  ...
  {k:"er22",    t:"ER22",        f:s=>flagged(fmt.er(s.er22), s.er22!=null&&s.er22<ER_HOT, "ER22 < "+ER_HOT+"，趋势效率低（震荡/过热）"), v:s=>s.er22},
  {k:"er55",    t:"ER55",        f:s=>flagged(fmt.er(s.er55), s.er55!=null&&s.er55<ER_HOT, "ER55 < "+ER_HOT+"，趋势效率低（震荡/过热）"), v:s=>s.er55},
...
function renderMajorTiles(){
  const majors=[...new Set(DATA.stocks.map(s=>s.major).filter(Boolean))].sort();
...
function refreshSubOptions(){
  // sub options limited to the selected major
  const pool=DATA.stocks.filter(s=>!fMajor||s.major===fMajor);
  const subs=[...new Set(pool.map(s=>s.sub).filter(Boolean))].sort();
  if(fSub && !subs.includes(fSub)) fSub="";   // drop sub that no longer belongs
