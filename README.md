...
- **数据源**：[Stooq](https://stooq.com) 的免费日线 CSV，无需 API key（类似 GoogleFinance 的免费替代）。
- **自动更新**：`.github/workflows/update.yml` 每个交易日收盘后自动抓数、计算、提交。
...
编辑仓库根目录的 **`config.csv`**，提交即可（提交会自动触发一次重算）：

```csv
ticker,exchange,benchmark,risk,breakout,type,name
AIR,NYSE,ITA,25,0.01,航空航天国防,AAR Corp
RKLB,NASDAQ,ITA,25,0.01,航空航天国防,Rocket Lab Corp
```

| 列 | 含义 | 说明 |
|----|------|------|
| `ticker` | 股票代码 | 带点的代码照写，如 `MOG.A`（程序自动转成 Stooq 的 `mog-a.us`） |
| `exchange` | 交易所 | 仅作记录，Stooq 不需要 |
| `benchmark` | 对标基准 | `SPY` / `QQQ` / `ITA` / `XLK`，决定「大盘过滤」用哪个 |
| `risk` | 可亏限额（$） | 单笔最大亏损，决定股数 |
| `breakout` | 突破确认 | `0.01` = 1%，越大入场缓冲越宽 |
| `type` | 类别 | 用于网页里的分类筛选（可留空） |
| `name` | 名称 | 仅显示用（可留空，会用代码代替） |

想加新的对标基准（比如某个行业 ETF），把它当成一行普通 `benchmark` 用即可，程序会自动抓它的收盘价并算 SMA100 / 回撤 / OK。
...
```bash
python engine/build.py            # 抓 Stooq 实时数据，写 docs/data/latest.json
