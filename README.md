...
编辑仓库根目录的 **`config.csv`**，提交即可（提交会自动触发一次重算）。**只需填 4 列**，公司名、对标 ETF、风险参数都自动处理：

```csv
ticker,exchange,major,sub
AIR,NYSE,国防航空航天,航空航天国防
RKLB,NASDAQ,国防航空航天,航空航天国防
```
...
- **对标 ETF（大盘过滤）**：按小类自动选 —— `航空航天国防 → ITA`，其余（通讯/电信/软件）→ `XLK`。
  想给某只单独指定，加一列 `benchmark` 写 `ITA`/`XLK`/`SPY`/`QQQ` 即可覆盖。
