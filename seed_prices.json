...
const COLS = [
  ...
  {k:"type",    t:"类别", l:true,  f:(s,st)=>st.type?`<span class="type-tag">${st.type}</span>`:"", v:(s,st)=>st.type||""},
...
let DATA=null, view="overview", sort={k:"signal",dir:1}, q="", typ="";
...
function buildTypeFilter(){
  ...
  [...new Set(DATA.stocks.map(s=>s.type).filter(Boolean))].sort().forEach(t=>{
...
function sortBy(k){
  ...
  else { sort.k=k; sort.dir = (k==="ticker"||k==="name"||k==="type"||k==="signal")?1:-1; }
}
...
function openDetail(tk){
  ...
  document.getElementById("detail").innerHTML = `
    <div class="d-head">
      ...
      ${st.type?`<span class="type-tag">${st.type}</span>`:""}
    </div>
...
document.getElementById("typeFilter").addEventListener("change",e=>{typ=e.target.value;render();});
